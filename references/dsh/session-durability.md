...
- **Approvals, queued input**: `tool/call` is logged before policy, the approval ask and guards. An ask appends log-only `approval/asked` then `approval/decided`; policy is the last `approval/policy` event; repair has no approval closer. `agent/inbox/*` is live-only; input is logged as `user/message` once claimed. `docs/tool-execution-pipeline.md`, `docs/subsystems/approval.md`, `docs/agent-lifecycle.md`
...
- **S17**: upstream's inbox is live-only, so unclaimed input dies with the process. MiniDSH's DURABLE inbox is deliberate; job completion through it has no upstream precedent.
...
- Whether any code persists the inbox; web archive semantics; cache row collection; the batching window's length; when `SessionOwnershipLostError` fires.
