...
- **S16, audit of attempts.** Stated limit: "durable execution intent, not exactly-once effects"; the other answer to an uncertain effect is a provider idempotency key off `exec.callId` (path unrecorded), which MiniDSH's `callId` keying leaves open. `packages/session/session-checkpoint-policy/README.md`
