...
Size budgets, enforced by `scripts/check-docs.ts` in `pnpm check`: `docs/ARCHITECTURE.md` ≤ 94 KB, `docs/BLUEPRINT.md` ≤ 30 KB, `README.md` ≤ 32 KB (so the warning holds it under 30), each file under `references/` ≤ 12 KB and the folder ≤ 120 KB (a reference file that outgrows its budget has started copying). **The architecture ceiling sits just above what its rules alone measure, never below: a budget under the contracts is an order to delete them.** It was 84 KB against the ~74 KB measured in 2026-09; S14 raised it to 94 against ~85, because that document is now contract per clause. Past 90% of a ceiling, the session that touches the document compacts existing sections before adding a line — but compaction is a lever with a floor, not an unlimited one. S14 measured it: three independent compressors under binding per-section maxima, told never to delete a claim, freed 0.37% of `ARCHITECTURE.md`. A session that measures such a floor reports the number and asks for the ceiling; it does not cut contracts to reach one, and it does not move a budget on its own.
