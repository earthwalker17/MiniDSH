...
- **Read a stored session without resuming it.** `minidsh sessions verify <id>` says whether a log is intact — `ok`, `interrupted` (a crash a resume will close) and `torn` exit 0; `damaged`, `invalid` (a rule the runtime holds every write to is broken) and a newer, unreadable format exit 1 — and `sessions inspect <id>` reports what each part of the log was written under, the authority it ends under, undecided approvals, queued input, children, cost, and exactly what a resume would append. Neither takes the session's lock nor writes a byte, so both are safe on a log another process holds. `--json` on each.
- **Salvage a damaged session.** `minidsh fork <id> --salvage` branches a log whose bytes are unreadable past some point from what is still readable, leaving the original untouched; every tool call it cannot account for is marked outcome-unknown, and the fork records where its source stopped.
- **The audit shows what calls did.** `sessions show --audit` now lists each file write and shell command the providers recorded (path and hash, mode, enforcement, exit status), each call whose outcome was lost with a crash, and, for a crashed session nobody has resumed, what a resume would record — marked as such.
...
- **`--accept` on `serve`, `web` and `chat` now does something.** It was accepted, advertised and dropped before reaching the sandbox, so wire-hosted sessions kept refusing. It now applies, and — like the sandbox mode and approval policy — the acceptance a session runs under is written into its log, so a stored session says what it accepted and a resume keeps it. A delegated child resumed under a weaker deployment no longer falls back to it for a mode it narrowed into.
