...
describe('headless runner (real composition, scripted model)', () => {
  ...
  it('an agent created and disposed without a prompt leaves no session file under the full composition', async () => {
    ...
    try {
      ...
      expect(abandoned.agent.session.events.map((event) => event.type)).toEqual(['session/lifecycle', 'agent/options', 'approval/policy', 'sandbox/mode', 'composition/applied'])
      ...
      expect(lines.slice(1, 6).map((line) => (JSON.parse(line) as { type: string }).type)).toEqual([
        'session/lifecycle',
        'agent/options',
        'approval/policy',
        'sandbox/mode',
