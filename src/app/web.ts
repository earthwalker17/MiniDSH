...
import { protocolPlugin, type ProtocolConfig } from '../capabilities/protocol/index.ts'
 ...
 */
function authoritiesFor(bind: string, port: number): ReadonlySet<string> {
 ...
 */
function sameAuthority(request: IncomingMessage, authorities: ReadonlySet<string>): boolean {
  const host = request.headers.host
  if (host === undefined || !authorities.has(host)) return false
  const origin = request.headers.origin
  if (origin === undefined) return true
  return authorities.has(origin.replace(/^https?:\/\//, ''))
}
...
export async function startWebHost(options: WebOptions): Promise<WebHostHandle> {
  ...
  const server: Server = createServer((request: IncomingMessage, response: ServerResponse) => {
    ...
    if (given !== null) {
      ...
      if (digest.length !== tokenDigest.length || !timingSafeEqual(digest, tokenDigest)) {
