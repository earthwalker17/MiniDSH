...
export type Carrier = StreamCarrier
...
export const protocolPlugin: Plugin<ProtocolConfig> = {
  ...
  apply(ctx, config) {
    ...
    const host = new ProtocolHost(ctx, {
      ...
      // A stream carrier's client IS this process's reason to run: when it
      // hangs up, the host is done. A socket carrier says otherwise.
      closeWithLastClient: true,
    ...
    ctx.effect(() => {
      const transports = carriers.map((carrier) => {
