...
This is the architecture contract: what each layer owns, what it may not own, where a new thing goes. History lives in `BLUEPRINT.md` §4 (beside this file) and the commits; usage in the root `README.md`; §12 lists deliberate differences from DeepSeek Harness (DSH), §13 what is knowingly missing. Section numbers are cited from code comments and are stable. Size budget: 94 KB, enforced by `pnpm check` (CLAUDE.md §5) — raised from 84 in S14, which measured that three independent compressors at binding per-section maxima freed 0.37% of this file. Compaction has a floor here; past 90% the answer is a measurement and a decision, not a third rewrite.
...
| `capabilities/<x>` | `kernel`, `core` (never `core/loop`; the gate cannot tell a Definition from a core helper); never another capability, never `app` |
| `app` | anything |
...
- **Services**: `provide(key, value)` is an effect; a key is claimed once per context chain (a child may shadow). `get(key)` is strict (only `inject`ed or self-provided); `tryGet` is optional. Keys: typed tokens (`serviceKey<T>`), not declaration merging. Deployment-global registries refuse a scoped owner.
- **Plugins**: `{ name, inject?, config?, apply(ctx, config) }`; `config` is a structural `{parse(unknown) → C}` contract (a zod schema qualifies; the kernel imports none, §12), parsed BEFORE `apply`; a bad row fails `PLUGIN_CONFIG` naming the plugin. `pending` until every injected key is provided, then `loading → active`; a vanished dependency unloads, a replaced provider reloads; transitions serialize per instance. Effects unwind before dependents unload: **a disposer must not read the services its plugin injected**. `settle(filter?)` awaits quiescence and reports pending plugins with their unmet keys and failed ones with their errors.
...
|---|---|---|
| `sessions` | `Session` = header + append-only log + surface + derivation; commit = validate → observe → push → deliver (an invariant rejects pre-commit); tiers (§4); `create/publish/get/list/flush/detach` (`publish: false` defers announcement: **session publication follows agent publication**; `detach` CLOSES the session before `session/disposed`: a later append throws `SESSION_CLOSED`); `Session.origin` (`new \| seeded \| resumed`, live-only); `session/created\|event\|flush\|disposed\|end-seed` (`event` contained, `flush` parallel); `repairInterruptedTail` (the session's OWN closers only; `core/agent` composes the rest, §4); folds `deriveEventMessage/foldRequestHeader/foldRequestContext/foldLastAssistantText/foldLastTurnEnd/sliceForkSeed` | persistence backends, UI state, model |
...
| `agents` | `Agent{id, session, status, ctx, options, configure, world, send/followup/steer/inject, cancel, whenIdle}`; `create/resume/fork/get/list`; `repairTail`, the static closer list every resume and cold fork runs (§4). Creation: TRANSACTION (session unpublished → scope → agent → opening `agent/options` → `setup` → register → publish; failure rolls back unannounced), one memoized disposal (owner+loop). `Agent.options` folds the BASE route; `configure(partial)` is the durable switch (one `agent/options{change}`, §4) and drops unrestated sampling knobs (whether a model honours one is adapter-owned). `resume` = `load` (refusing `damaged`, as `fork` of a STORED source does) → repair → seed → same transaction, `origin:'resumed'`; model config = caller overrides > seed's folded `agent/options` > its `request/header` > surface defaults. `fork(owner, source, boundary?)` slices a live or repaired log into a child (`parentId`/`seedLength`), refusing a boundary below a delegated child's opening stamps (§7). `resolveCallConfig(agent, call)` = ONE route resolution. Inbox `inbox/spliced`+`foldInbox` (§4) | driver |
...
**Vocabulary** (29 kinds): `turn/start`, `turn/end{reason: completed | blocked | cancelled | error{code,message} | max-tokens | max-steps | interrupted}`, `step/start`, `step/end`, `user/message`, `agent/options{options, reason: initial|change|resume}`, `request/header{provider, model, reasoningEffort?, maxTokens?, temperature?, system, tools; reason}` (written only when it changes, so a provider prefix cache survives restarts), `request/context{provider, model, contextWindow?, inputModalities?}` (`inputModalities` absent MEANS text only), `assistant/chunk{turn, step, attempt, chunk}` (trace), `assistant/message{message, usage?, interrupted?}`, `tool/call`, `tool/dispatch{turn, step, callId}` (the gate passed, the body is about to run: §6), `tool/result`, `effect/recorded{callId} & (fs-write{path, bytes, sha256} | shell-command{exitCode?, durationMs, mode, enforcement, timedOut?})` (below), `inbox/spliced`, `approval/asked{id}`, `approval/decided{id, outcome}`, `approval/policy{policy, reason}`, `sandbox/mode{mode, enforcement, reason}`, `authority/preset{name}` (intent, §7), `composition/applied{hash, layers, rows}` (hashed, never inlined), `llm/aux-call{purpose, provider, model, maxTokens?, inputSeqs?, usage?, outcome}`, `compaction/start|applied|end`, `subagent/start|end` (PARENT's log), `session/title{title, messageSeqs, source: fallback|provider|user}` (log-only, last-wins; only `fallback` is produced today, but a rename must stay distinguishable from a generated title), `session/end-seed`.

Header `{version, id, createdAt, cwd, parentId?, seedLength?, delegatedBy?, delegatedByCallId?, delegationDepth?, agentPreset?}`, beside the log (`delegatedByCallId` is WHICH parent call made this child: the child is published and materialized before the parent appends `subagent/start`, so a host death between them leaves a child on disk with nothing about it in the parent log, and two children of one step are otherwise indistinguishable from this side; it travels through a resume and a fork as the lineage beside it does). `SESSION_FORMAT_VERSION = 0`, additive; a NEWER stored version refuses loudly at load and attach. Unknown types load as opaque log-only records, skipped by derivation, only while they carry neither `surfaceOp` nor `sourceEventSeqs` (§13); plugin state is a log-only kind, never surface.
...
**Crash repair closes everything the log left open.** `core/agent/repair.ts` composes a STATIC list of three pure closers, each given the original events, its starting seq and one shared timestamp (the last real event's), so a cold read and a durable repair produce identical bytes — and a second crash before they reach disk recomputes them. Innermost first: every unpaired `subagent/start` → `subagent/end{interrupted}` (the delegation tool owes its end on every exit path it has, so an unpaired one IS a host death; no `usage`, a closer may not invent a number); every unpaired `compaction/start` → `compaction/end{declined: unclosed}`, not `interrupted` (the commonest way to reach one is an ordinary disposal, where the provider hits `agent-gone` with the session detached and no append legal); then `repairInterruptedTail`. The bracket closers run with no turn open, which is what reaches a `/compact` bracket opened idle. Each sits beside the vocabulary it closes (`agent/events.ts`, `compaction/events.ts`), so the session learns no other package's brackets (§12).

**The surface closers.** The driver commits an assistant message with ALL its tool-call blocks and logs each `tool/call` only when its turn comes, so `repairInterruptedTail` answers every tool-call block of the open step, cancels the turn's undecided approvals, closes step and turn `interrupted` — a history every OpenAI-compatible wire accepts. The code is evidence, in order: no `tool/call` ⇒ `TOOL_NOT_STARTED`; a `tool/dispatch` OR a recorded effect ⇒ `TOOL_OUTCOME_UNKNOWN`; neither, in a log that records dispatches AND kept an event written after that `tool/call` ⇒ `TOOL_NOT_STARTED` — it died at its gate, the row §6's fact bought; anything else ⇒ `TOOL_OUTCOME_UNKNOWN`. Both conditions of the third row are load-bearing. A 1.0.0 log records no dispatch, so absence says nothing there. And the log is not fsynced, so a crash takes a SUFFIX: absence is only evidence where the writer demonstrably got past the call, because it would have written the dispatch before whatever survived after it. Without that, a power cut following a landed effect would read as "did not run" — the one error this prevents (§13). An unknown result names the effects recorded for that call IN THAT STEP (bounded by the step: nothing makes a call id unique across steps) and says to check current state and repeat only a read-only or idempotent action, never blindly.

**An effect is recorded by the code that caused it, AFTER it happens.** `effect/recorded` is written at the boundary by the provider that owns it — `fs-local` after a write lands (the canonical path the fence approved, bytes, sha256 of exactly those bytes), `shell-stdio` after a command returns (exit code, duration, the mode and enforcement it got) — from values that provider computed, never model prose, and never for a run it refused or never dispatched. **Presence is proof; absence proves nothing**: the append follows the effect, a failed append is swallowed rather than failing a write that landed, and nothing may read a missing record as "nothing happened" — nor as an inventory, which the model-facing rendering says in as many words. Two closed families, no key, no provider seam, no exactly-once claim (§13).
...
**Persistence is a subscriber**: `sessions/<id>.jsonl` under `MINIDSH_HOME` (§9), header as line 1. A fresh or forked session **materializes on its first conversation fact**; a `resumed` one attaches append-only; a torn final line moves to `.torn`, deeper corruption is `damaged` and refuses attach. `session/flush` (every durability checkpoint) rethrows a remembered write error, every time. No `fsync`: the modelled failure is a crash, which the page cache survives; a power loss lands in torn-tail repair only if it TEARS a line (§13).
...
**Durable before every action.** A CHECKPOINT is `session.flush()` (§4); a lost write ends the turn `turn/end{error, code:'DURABILITY_LOST'}` before the next paid request or tool effect, every remaining call answered, none run — the same shape as a cancellation, so the surface stays balanced. A call has TWO: after `tool/call`, so an attempted call is durable; and after the gate, before the body, so `tool/dispatch` is durable and its ABSENCE is evidence (§4). The second is the DRIVER's, through `ToolCall.onDispatch`, awaited inside the `tools/execute` TERMINAL continuation — inside, because a middleware answering without `next()` replaces the body and a fact before the waterfall would claim one that never ran; the driver's, because the call family keeps one writer and a lost write must reach the loop's state machine, not become an error RESULT a concluding call could close the turn `completed` around. The abort signal is re-checked on both sides of it (§12).

**The route is resolved once per step**, before the pre-step listeners, `request/context` written immediately after (so a pressure check measures against THIS step's route and window), and fixed for the step, retries included, so a header and its context record can never describe different requests; a switch lands at the NEXT step. The request is built per ATTEMPT: a recovery listener may change the log between attempts.

`agent/status` only on `idle ↔ running` transitions; one `AbortController` per turn; `cancel(cause)` clears the inbox and aborts; `whenIdle()` is quiescence; dispose = cancel → whenIdle → unwind scope → detach agent → detach session. The driver promise is fire-and-forget, reporting (`agent/error`) instead of rejecting unobserved.
...
**Deadlines are the pipeline's.** `ToolDefinition.timeoutMs` (omitted = registry default, `null` opts out) is enforced by the registry, since the kernel waterfall passes `next()` no arguments, clocked from *after* the gate; expiry ends the CALL `TOOL_TIMEOUT`, signal aborted, late result discarded; the body is abandoned, neither killed nor awaited: `whenIdle` is DRIVER quiescence. Consent INSIDE a body waits on `ToolExecution.callSignal` (cancellation, no deadline) and owns its clock (`timeoutMs: null`).
...
**One stamp.** `SandboxExecutionPolicy{mode, workspaceRoot}` resolves per call from `ctx.sandbox`, never caller-assembled, shared by every execution world; mode governs FILE EFFECTS only. The boundary needs no event: the immutable `SessionHeader.cwd` recorded at creation IS the root for every call in that session, which is why the wire may not choose it (§8). `writableRoots(policy)` is a CEILING for both families: grant LESS, never more. `resolve(request)` records the effective stamp when it differs and refuses an escalation past a ceiling. Precedence: approved one-shot escalation > folded `sandbox/mode` > composition default.
...
A surface injects only `agents` and `sessions` — plus `llm` for the catalog handshake, `sandbox`/`approval` for the authority control plane — owns transport and process exit, renders from `session/event`, holds no authority state: a switch it asks for is a durable event it reads back. Plain-text surfaces share ONE projection, `app/present.ts`, neutralizing control characters in model text (a terminal executes what it is written); the browser renders structured DOM per event type, a different projection duplicating no fold.
...
- `--help`/`-h` and `--version`/`-v` (`minidsh <version>`, answered before help, boot-free, from the one string `app/version.ts` reads out of `package.json` and gives both protocol hosts) exit 0; a preamble failure exits 2, except that `sessions` downgrades a broken settings file to a warning (a stored log must stay readable).
...
**Packaging.** Development and tests run the `.ts` source natively; Node won't strip types under `node_modules`, so what SHIPS is an emit: `pnpm build` (`scripts/build.ts`, `prepack`) runs `tsc -p tsconfig.build.json` with `rewriteRelativeImportExtensions`, copies the browser client's five files verbatim, then smoke-imports `dist/app/cli.js`. `package.json` ships `bin`, `dist`, `README.md`, `LICENSE`, `NOTICE` only — no `main`, no `exports`: a binary, not an importable surface (BLUEPRINT §3). `bin/minidsh.js` picks by package SHAPE, never staleness (`../src/app/cli.ts` beside it, else `../dist/app/cli.js`) and refuses below Node 24. `app/version.ts` resolves `../../package.json` from `import.meta.url`, valid in both layouts.

## 10. Verification

Tests mount real compositions through the kernel; only the model is scripted (`test-support/scripted-adapter`) or replayed (`test-support/llm-replay`: the script derived from `assistant/chunk` groups per (turn, step), EVERY terminally-finished attempt replayed, out-of-loop calls served from `llm/aux-call` records through a second cursor, every recorded call asserted consumed). **A live session log is its own test oracle** for the session that WROTE it, unless it holds a REPAIRED interruption (replay runs the interrupted call to completion); sessions are matched to logs in first-request order (§13). The protocol is tested byte-level over an in-memory duplex pair; the terminal's render folds are pure functions plus scripted runs over the loopback carrier; the browser client's window rules and row projection (§11) have unit tests, its DOM rendering does not.
...
`pnpm test:e2e` runs the eight live arcs (`DEEPSEEK_API_KEY`-gated; routing also `ANTHROPIC_API_KEY`): seven through a `minidsh serve` child (real stdio JSON-RPC), one through `minidsh web` (real WebSocket). Both harnesses bound every request with a deadline naming the method; the child lacks `MINIDSH_MODEL`, its settings its only route input.

| arc | what it proves |
|---|---|
| `live` | completes; mid-turn SIGKILL repaired (a synthetic result is `TOOL_OUTCOME_UNKNOWN` iff a `tool/dispatch` names its call, and no bracket is left open); second process finishes; keyless replay |
| `authority` | a workspace edit lands and its `effect/recorded` sha256 matches the bytes on disk, one outside is refused and probed ABSENT; every shell record carries the enforcement its branch predicts, and an unconfined host records only the escalated command; the shell BRANCHES on the enforcement `initialize` reports (unconfined: refused, the model escalates, the client approves, the approved call's own result proves it ran; confined: no decision, no host file, every approval `rejected`, the attempt scoped to that turn and naming its target); then `read-only`: the next write attempted and refused |
...
**Validation is proportionate (CLAUDE.md §10):** the full gates plus, once and on an idle machine, the arcs a change touches; every arc three times per host and `live.yml` at `.5` checkpoints, release candidates and changes to the loop, the log or the protocol host. Green is not a diagnosis; a stalled arc names what the session was parked on, and every asserted count names its producer. **A live arc's premise is an assumption about the model, and it decays silently**: with ONE separating assertion, ask what a model knowing nothing would answer.
...
| an event's human line | one branch in `app/present.ts`, narrowed with `matches()`, AND one case in `app/web/rows.js`; `rows.test.ts` fails until both exist |
...
| an attachment kind | a RASTER type: a media type + header probe in `attachments-local`, not a seam; a non-image kind is a vocabulary change (`AttachmentRef`, `ModelModality`, `ContentBlock`, the serializers, the meter) |
...
| an effect worth recording | a third family in `core/effects` (§4), written by the PROVIDER after the effect, from what it computed; never a tool reporting on itself, never a fence |
| a durable step in the tool pipeline | `ToolCall.onDispatch`: the only hook that runs between the gate and the body and cannot replace it (a `tools/execute` middleware sits there too and may, §6), and the loop owns what it writes |
...
Most DSH claims here have a pinned verdict in `references/assumptions.md`: re-check first.
...
- **Concurrency.** Sequential tools, one foreground child, no background jobs, as DSH's `minimal` preset (its `standard` preset delegates continuable-background by default).
- **The wire.** One protocol (DSH's SDK shape) where DSH ships five composition profiles (`sdk` and `sdk-minimal` share a wire), plus `session/cancel`, `approval/answer`, `session/authority` and a catalog-returning `initialize`; no lower-bound attach cursor (DSH's shape too), but a both-ends-bounded gap repair and a host page ceiling; pages without the trace tier (DSH has none: its log-only events ride in pages, unbounded in bytes); watch sets and tier-aware drops where DSH's follower queues are uncapped; approvals settle `unavailable` when unseen, where DSH's, once an answerer is composed, showed no timeout (§8).
- **Confinement.** Two backends, not DSH's four local ones, the bwrap profile adopted flag for flag; no Landlock (a native launcher a no-native, no-build tree cannot carry); no Windows backend (§13). Unlike DSH (§7): a functional probe, since `enforcement` is recorded before any command runs; `-D` Seatbelt parameters, not an SBPL escaper; a POSIX session per confined child on both backends.
- **The ceiling.** `writableRoots` grants the workspace root ONLY (DSH's shared list adds `/tmp` and `os.tmpdir()`, where the tests live; only its Seatbelt and fs fence consume it) and binds BOTH families where DSH lets each spell its own. `SandboxEnforcement` keeps `none`: a supported posture, not an error.
- **Durability.** A durable `tool/dispatch` after the gate where DSH puts the same fail-closed barrier in the same place and writes no fact for a TOP-LEVEL call (its `tool/ptc-dispatch-start` pair is for PTC sub-calls and fires BEFORE their gate) (so its repair classifies on a `tool/call` logged BEFORE the gate, and a crash mid-consent reads as may-have-run); a recorded EFFECT where DSH computes per-turn git snapshots and content-addressed copies and keeps them on the Host, by stated decision, so neither claims exactly-once and only one survives the process. Plain JSONL through one held descriptor with a session-lifetime write lease, a `.torn` sidecar where DSH salvages in place; JSON config, two layers, read at boot, not YAML, four, watched; a per-boot `composition/applied` record DSH lacks; no session deletion in either.
- **Delegation and routing.** A delegation opening is an enforced ceiling where DSH trusts the pin alone; `subagent/start|end` live in the parent's LOG and have no upstream counterpart (DSH's subagent family is catalog and descriptor records, none paired). Repair closes every unpaired bracket where DSH closes none and says why — a bracket's vocabulary is its owner's, and a lifetime marker lets each owner judge its own dead openers — which binds only where that owner is a plugin; both of MiniDSH's are core's. `delegatedByCallId` answers a question DSH declines outright (no record, receipt, header or descriptor field points back at a spawning call); model roles and a durable BASE route are seams here, not upstream.
- **Context.** No deterministic tool-result pruner: DSH's runs only once a compaction trigger qualifies, and MiniDSH's own tools bound their output (spill, the editor cap, the shell excerpt). Recall is GATED, not opt-in: DSH's `session-query` (a service, a SQLite FTS5 backend, five model-facing tools) ships with search OFF (`openAt: never`) and its tools in no composition, for the cost its README names (fixed guidance plus five tool schemas on every request); here one tool over THIS SESSION's shadowed spans, never cross-session by `cwd` equality.
...
- **Not built.** No PTC (DSH renamed Code Mode TO it), model-written packages (DSH: since 2026-09-16 persistent `plugin_manager` installs in its opt-in `cordis` preset; `node:vm` is no boundary), MCP bridge (DSH's is inert until a server is configured), session search index, workspace config layer, per-user identity.
...
- Windows has no confinement backend and will not (DSH's is a `WRITE_RESTRICTED` token, `partial`, the `Everyone` SID kept, an NTFS hard-link escape, standing ACL changes): every shell command costs an escalation (two model steps, consent to the WIDEST mode, a throwaway shell keeping no cwd or environment); a delegated child can run no shell; headless without `--approve` cannot run one; escalated descendants are not reaped as a group; `danger-full-access` stops the prompts by dropping the in-process fs fence, Windows' only confinement.
- On POSIX an UNWRAPPED persistent child (`danger-full-access`, `confinement: none`) is not detached: a timed-out or backgrounded command's descendants outlive it.
...
- The fs fence is a policy check in trusted code over a model-supplied path; a confined shell is a real kernel boundary — two families enforcing one mode differently, escalation voluntary either way (the model must ask). `--audit` projects decisions, not attempts: a refused command the model never escalated is in the transcript, not the audit.
...
- The meter changes units at a compaction (safe in direction), meters a prompt or tool-set change one step late, charges an image on a text-only route as an image, may ask a smaller-model role for a summary larger than it takes.
...
- A pid wraparound inside one agent's lifetime could make pgid reaping hit an unrelated group: an unreachable edge needing millions of process creations between.
...
- **Crash repair is as sharp as the log is (§4).** Two shapes read `TOOL_OUTCOME_UNKNOWN` where the truth is narrower: a call killed inside its gate before a log's FIRST `tool/dispatch`, and one whose `tool/call` is the last surviving line. A per-lifecycle provenance fact and a real fsync barrier (S16) each remove one. `tool-shell` takes its escalation consent INSIDE the body, so a crash while a person deliberates is unknown too — correctly, with "no effect recorded" beside it. A `subagent/end` closer sums no cost. An unpaired `compaction/start` cannot tell a crash from the disposal that also produces one, hence `unclosed`. A closer reaches only the tail turn for surface events and the whole log for brackets.
- **What an effect record does not cover.** Only what crosses `ctx.fs` and `ctx.shell`: a shell command's own writes, the spill store and the attachment store are outside it. It can also over-report by a hair — a persistent shell that died between two commands answers the second as an ordinary run, since liveness is only observed one tick later — which pushes repair toward unknown, the safe way. A body the deadline abandoned (§6) can append one AFTER the result that named it — the one fact that arrives out of order.
- The format version is stamped once (line 1), never restamped at attach; an unknown kind carrying `surfaceOp` THROWS in the seed rather than refusing by version; `freezeEnvelope` drops unknown envelope fields; whole lines lost to a power cut leave a valid shorter log.
...
- `session.view` is not re-sent on a status change, so its `status` goes stale (the browser's override is wrong after a reconnect). One watch set is subscription, control plane and answerer candidacy: narrowing blinds a client to other sessions' approvals.
- `minidsh web` has no graceful stop (no signal handler; `shutdown` refused on a socket): repair and lease reclaim fall to the next process. No flag or layer reaches the `workspaces` seam: `workspaceRoots` is always `[cwd]`.
...
- No OpenAI or Moonshot adapter. A provider catalog is a snapshot: an unknown ANTHROPIC id gets conservative facts (200K window, 8192 output cap), a divided family needs a row per member, and `llm-deepseek` answers 1M and its full effort set for ANY id.
...
src/core/   json.ts · ids.ts · scope.ts · text.ts (printableText)
  ...
  agent/    types.ts · events.ts · index.ts (Agent, ctx.agents, Inbox, resolveCallConfig, mergeAgentOptions) · repair.ts (the closer list) · invariant.ts
  ...
  compaction/ events.ts · index.ts (planCompaction)
  effects/  events.ts · index.ts (the effect vocabulary; no service)
  metering/ · spill/ · attachments/ · invariants/ · persistence/ · settings/
src/capabilities/
...
src/app/    home.ts · compose.ts (rows, applyPatches, the builtin catalog) · config.ts (the disk layers) · settings.ts
            ...
            headless.ts (bootComposition) · serve.ts · web.ts · web/ (index.html · style.css · wire.js · rows.js · app.js)
