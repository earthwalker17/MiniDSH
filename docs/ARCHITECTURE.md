...
**Persistence is a subscriber**: `sessions/<id>.jsonl` under `MINIDSH_HOME` (§9), header as line 1. A fresh or forked session **materializes on its first conversation fact**; a `resumed` one attaches append-only; a torn final line moves to `.torn`, deeper corruption is `damaged` and refuses attach. `session/flush` (every durability checkpoint) rethrows a remembered write error, every time. No `fsync`: the modelled failure is a crash, which the page cache survives; a power loss lands in the torn-tail repair only if it TEARS a line (§13).
...
**Deadlines are the pipeline's.** `ToolDefinition.timeoutMs` (omitted = registry default, `null` opts out) is enforced by the registry, because the kernel waterfall passes `next()` no arguments, clocked from *after* the gate; expiry ends the CALL `TOOL_TIMEOUT`, signal aborted, late result discarded; the body is abandoned, neither killed nor awaited: `whenIdle` is DRIVER quiescence. Consent INSIDE a body waits on `ToolExecution.callSignal` (cancellation, no deadline) and owns its clock (`timeoutMs: null`).
