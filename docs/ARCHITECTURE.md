...
**Deadlines are the pipeline's.** `ToolDefinition.timeoutMs` (omitted = registry default, `null` opts out) is enforced by the registry, because the kernel waterfall passes `next()` no arguments, clocked from *after* the gate; expiry ends the CALL `TOOL_TIMEOUT`, signal aborted, late result discarded; the body is abandoned, neither killed nor awaited by disposal: `whenIdle` is DRIVER quiescence. Consent INSIDE a body waits on `ToolExecution.callSignal` (cancellation, no deadline) and owns its clock (`timeoutMs: null`).
...
Each DSH claim here is a dated row in `references/assumptions.md`: re-check it there before leaning on it.
...
- **Context.** No deterministic tool-result pruner: DSH's runs only once a compaction trigger qualifies, and the shipped tools bound their own output. Recall is GATED, not opt-in: DSH's `session-query` (a service, a SQLite FTS5 backend, five model-facing tools) is mounted with full-text search OFF (`openAt: never`) and no model tools, for the prompt cost its README names; here one tool over THIS SESSION's shadowed spans, never cross-session by `cwd` equality.
...
- **Not built.** No PTC (DSH renamed Code Mode TO it), model-written packages (DSH: since 2026-09-16 persistent `plugin_manager` installs in its opt-in `cordis` preset, `node:vm` explicitly not a security boundary), MCP bridge (DSH's is inert until a server is configured), session search index, workspace config layer, per-user identity.
...
- `minidsh web` has no graceful stop (no signal handler; `shutdown` refused on a socket): no dispose-to-idle, so repair and lease reclaim fall to the next process. No flag or layer reaches the `workspaces` seam: `workspaceRoots` is always `[cwd]`.
...
`scripts/count-lines.ts` measures the tree; the 1.0.0 figures are in BLUEPRINT §4.
