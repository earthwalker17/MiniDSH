...
- **Surfaces.** A terminal ships (DSH deleted its TUI) as a protocol client; the browser samples scroll ownership rather than keeping a write ledger (`app/web/app.js` says when that is sound).
