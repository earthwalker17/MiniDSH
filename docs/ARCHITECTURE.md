...
This is the architecture contract: what each layer owns, what it may not own, where a new thing goes. History lives in `BLUEPRINT.md` §4 (beside this file) and the commits; usage in the root `README.md`; §12 lists deliberate differences from DeepSeek Harness (DSH), §13 what is knowingly missing. Section numbers are cited from code comments and are stable. Size budget: 84 KB, enforced by `pnpm check` (CLAUDE.md §5). It is FULL: S14 measured that three independent compressors at binding per-section maxima freed 0.37%, so the next contract needs a decision about the budget, not a compaction (BLUEPRINT §3).
