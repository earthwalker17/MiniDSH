...
Header `{version, id, createdAt, cwd, parentId?, seedLength?, delegatedBy?, delegationDepth?, agentPreset?}`, beside the log. `SESSION_FORMAT_VERSION = 0`, additive; a NEWER stored version refuses loudly at load and attach. Unknown types load as opaque log-only records, skipped by derivation, only while they carry neither `surfaceOp` nor `sourceEventSeqs` (§13); plugin state is a log-only kind, never surface.
...
**Durable before every action.** A CHECKPOINT is `session.flush()` (§4); a lost write ends the turn `turn/end{error, code:'DURABILITY_LOST'}` before the next paid request or tool effect, every remaining call answered, none run — the same shape as a cancellation, so the surface stays balanced.
...
- **Crash repair closes turns, steps, calls and approvals only:** after a host death `compaction/start` and `subagent/start` stay unpaired (the child's cost summed nowhere), a bracket opened outside a turn is unreachable, and a call parked at an undecided approval closes `TOOL_OUTCOME_UNKNOWN` though its body never ran. Nothing records the EFFECT.
