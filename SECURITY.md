...
- **Shell commands** are confined by the operating system where a backend exists — bubblewrap on Linux, Seatbelt on macOS — and the backend is *probed* functionally before the harness claims it. Windows has no backend: there every shell command costs one approval — the harness refuses to run it confined, the model asks for a one-shot escalation, and a person grants it (`--approve` grants them all on a headless run). Confinement governs **file effects only**. A confined command still reaches the network and inherits the environment MiniDSH was started with, provider keys included.
